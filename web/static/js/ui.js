class GameUI {
    ...
    applyPreferences() {
        ...
        // Применяем анимации
        this.setAnimationState(this.preferences.animation_speed);
        
        // Применяем звук
        this.setSoundState(this.preferences.sound_enabled);
        
        // Применяем специальные возможности
        document.body.classList.toggle('accessibility-mode', 
            this.preferences.accessibility_mode);
        
        // Применяем язык
        document.documentElement.lang = this.preferences.language;
        
        // Обновляем элементы управления настройками
        this.updatePreferencesControls();
    }

    updatePreferencesControls() {
        // Обновляем переключатель темы
        if (this.elements.themeToggle) {
            this.elements.themeToggle.querySelector('input').checked = 
                this.preferences.theme === 'dark';
        }

        // Обновляем контроль анимации
        if (this.elements.animationControl) {
            this.elements.animationControl.value = this.preferences.animation_speed;
        }

        // Обновляем переключатель звука
        if (this.elements.soundToggle) {
            this.elements.soundToggle.checked = this.preferences.sound_enabled;
        }

        // Обновляем другие элементы управления настройками
        if (this.elements.accessibilityMode) {
            this.elements.accessibilityMode.checked = 
                this.preferences.accessibility_mode;
        }
    }
    ...
    async initializeElements() {
        ...
        try {
            const [mainMenu, gameContainer, playersContainer] = 
    ...
    setupEventListeners() {
        ...
        this.addEventHandler(this.elements.menuButton, 'click', 
            () => this.toggleMenu());
        
        this.addEventHandler(this.elements.startGame, 'click', 
        () => this.game.startGame());
    ...
    // Обработчики формы настроек
    if (this.elements.preferencesForm) {
        this.addEventHandler(this.elements.preferencesForm, 'submit', 
            (e) => this.handlePreferencesSubmit(e));
    }
}
...
async handlePreferencesSubmit(event) {
    ...
    const updates = {
        ...
        animation_speed: formData.get('animation_speed'),
        ...
        keyboard_shortcuts: formData.get('keyboard_shortcuts') === 'true'
    };

    const success = await this.savePreferences(updates);
    if (success) {
...
setupSettingsListeners() {
    ...
    if (this.elements.soundToggle) {
        this.addEventHandler(this.elements.soundToggle, 'change', (e) => {
            this.setSoundState(e.target.checked);
        });
    }
    ...
    if (this.elements.accessibilityMode) {
        this.addEventHandler(this.elements.accessibilityMode, 'change', (e) => {
            this.savePreferences({ accessibility_mode: e.target.checked });
        });
    }
}
...
setAnimationState(value) {
    ...
    this.savePreferences({ animation_speed: value });
}
...
setSoundState(enabled) {
    ...
    this.savePreferences({ sound_enabled: enabled });
}
